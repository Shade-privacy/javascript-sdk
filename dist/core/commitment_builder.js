...
export class CommitmentBuilder {
     ...
     */
    async buildCommitment(secret, nullifier, assetId, amount) {
        // Validate inputs
     ...
     */
    validateInputs(secret, nullifier, assetId, amount) {
     ...
     */
    validateNullifierInputs(nullifier, secret) {
        if (nullifier < 0n || nullifier >= (1n << 256n)) {
            throw new Error('Nullifier must be a 256-bit value');
        }
        if (secret < 0n || secret >= (1n << 256n)) {
     ...
     */
    async reconstructCommitment(secret, nullifier, assetId, bucketAmount) {
     ...
     */
    decomposeToBuckets(amount) {
        const buckets = [];
        let remaining = amount;
        // Use buckets in descending order
        const sortedBuckets = [...SHADE_DOMAIN.AMOUNT_BUCKETS].sort((a, b) => Number(b - a));
     ...
     */
    async verifyCommitment(commitment, secret, nullifier, assetId, amount) {
        try {
            const bucketAmount = this.findBucket(amount);
            const recomputed = await this.reconstructCommitment(secret, nullifier, assetId, bucketAmount);
...
//# sourceMappingURL=commitment_builder.js.map